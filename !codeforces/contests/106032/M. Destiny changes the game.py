import sys
# ใช้ fast I/O: อ่านทีละบรรทัดจาก buffer เร็วกว่า input() ปกติ (ลด TLE จาก
# I/O ได้เยอะ)
...
for _ in range(t):                # วนทำทีละเทส
    ...
    # person[0] = นับตัวอักษรของ Alice, person[1] = ของ Bob (ดัชนี 0..25 =
    # 'a'..'z')
    person = ([0] * 26, [0] * 26)
    # ทั้งคู่เริ่มด้วย "a" 1 ตัว → เพิ่มนับ 'a' ให้ Alice
    person[0][0] += 1
    person[1][0] += 1             # เพิ่มนับ 'a' ให้ Bob

    # ความยาวสตริงหลังเรียง (จริง ๆ คือความยาวรวม) ของ Alice/Bob (เริ่ม 1
    # เพราะ "a")
    s_len = [1, 1]

    for __ in range(q):           # ประมวลผลแต่ละ operation
        op, x, k = input().split()  # อ่านสามส่วน: op (1=Alice, 2=Bob), สตริง x, จำนวนครั้ง k
        # แปลงเป็น 0/1 เพื่อใช้เป็นดัชนี (0=Alice, 1=Bob)
        op = int(op) - 1
        k = int(k)                # k เป็นจำนวนเต็ม

        # อัปเดตการนับตัวอักษร: ต่อ x จำนวน k ครั้ง
        # ทำโดย "บวกทีเดียว" ต่ออักขระแต่ละตัวใน x ด้วยค่า k (ไม่ต่อสตริงจริง →
        # ประหยัดเวลา/หน่วยความจำ)
        for c in x:
            # เพิ่มตัวอักษร c ให้คนที่ op ถืออยู่ ด้วยจำนวน k
            person[op][ord(c) - ord('a')] += k
            # ความยาวรวมของฝั่งนี้เพิ่มขึ้น k ต่ออักขระ 1 ตัว → รวมแล้วได้
            # |x|*k
            s_len[op] += k

        # คัดลอกความยาวคงเหลือ (เอาไว้ใช้ตัดสินเคส prefix: สั้นกว่าเล็กกว่า)
        rem_len = [s_len[0], s_len[1]]
        ...
        for i in range(26):
            # จำนวนตัวอักษรตัวที่ i ของ Alice (หลังรวมทุก op แล้ว)
            a = person[0][i]
            b = person[1][i]                  # ของ Bob

            # จำนวนที่ "จับคู่เท่ากัน" ได้ ณ ตัวอักษรนี้ (เทียบเหมือนหักคู่ออก)
            cut = min(a, b)
            # หักออกจากความยาวคงเหลือของ Alice
            rem_len[0] -= cut
            ...
            if (a < b and rem_a == 0 and rem_b > 0) or (a > b and rem_b > 0):
                # เคสที่ Alice เล็กกว่า (prefix หรือมีตัวอักษรที่เล็กกว่าฝั่ง
                # Bob ในตำแหน่งแตกต่างแรก)
                print("Alice")
                break
            ...
            # หมายถึง นับตัวอักษรทุกตัวเท่ากันหมด → สตริงหลังเรียงเหมือนกัน →
            # "Tie"
            print("Tie")
